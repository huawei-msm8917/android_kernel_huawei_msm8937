...
#define S5K5E8_LSC_PAGE10_REG_NO    24
#define S5K5E8_LSC_PAGE10_NO     10
...
static int ags_s5k5e8_read_otp_lsc(struct msm_sensor_ctrl_t* s_ctrl,uint16_t  lsc_addr,int checksum_otp)
{
	...
	ags_s5k5e8_otp_set_page(s_ctrl,S5K5E8_LSC_PAGE10_NO);//page 10
	rc = ags_s5k5e8_otp_continuous_read(s_ctrl,0x0A04,S5K5E8_LSC_PAGE10_REG_NO,//24
								buf, &sum_lsc);
	sum_lsc1 += sum_lsc;
	...
	CMR_LOGW("%s ,lsc success verify checksum ,checksum=0x%x,OTP_checksum=0x%x\n",
					__func__, checksum,checksum_otp);
	return 0;
}
