					...
					size_t nbytes, loff_t *pos)
{
	...
	struct i2c_client *client = pakit_priv->i2c_priv[0]->i2c;
	int ret = 0;
	void *kern_buf;
	...
	ret = pakit_priv->ioctl_ops->i2c_read(client, (char *)kern_buf, (int)nbytes);
	if (0 > ret) {
			 ...
			 const char __user *buf, size_t nbytes, loff_t *ppos)
{
	...
	ret = pakit_priv->ioctl_ops->i2c_write(client, (char *)kern_buf, (int)nbytes);
	if (0 > ret) {
...
static int smartpakit_ctrl_do_ioctl(struct file *file, unsigned int cmd, void __user *arg, int compat_mode)
{
	...
	switch (cmd) {
		// smartpa with dsp, ex. tfa9895
		case I2C_SLAVE:
		case I2C_SLAVE_FORCE:
		{
			unsigned long addr = (unsigned long)arg;

			if (pakit_priv->pa_num != 1) {
				return -EINVAL;
			}
			client = pakit_priv->i2c_priv[0]->i2c;
			...
			*/
			if ((addr > 0x3ff) ||
				(((client->flags & I2C_M_TEN) == 0) && addr > 0x7f))
				return -EINVAL;
			/*
...
static int smartpakit_parse_dt_switch_ctl(struct platform_device *pdev, smartpakit_priv_t *pakit_priv)
{
	...
	for_each_property_of_node(node, pp) {
		...
		if (ctl[i].gpio < 0) {
			...
			ret = of_property_read_u32(node, pp->name, &ctl[i].gpio);
			if (ret < 0) {
	...
	if (of_property_read_bool(node, gpio_state_default_str)) {
		...
		ret = of_property_read_u32_array(node, gpio_state_default_str, gpio_state, (size_t)(long)count);
		if (ret < 0) {
